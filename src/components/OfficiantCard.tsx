...
export default function OfficiantCard({ officiant }: OfficiantCardProps) {
  ...
  return (
    <Link href={`/officiant/${officiant.id}`}>
      <article className={`card group cursor-pointer ${officiant.isFeatured ? 'ring-2 ring-[var(--primary)] ring-opacity-50' : ''}`}>
        {/* Featured Badge */}
        {officiant.isFeatured && (
          <div className="flex items-center gap-1.5 text-xs font-medium text-[var(--primary)] mb-3 -mt-1">
